...
function Room(manager, id, sockets, wait_time, auction_time) {
    ...
    room.players = sockets.map((s, index) => {
        return new Player(s.id, index);
    });
    ...
    room.ready = [false, false, false, false, false];
    ...
    room.lineup_index = [2, 3, 4, 5, 7, 8, 9, 10, 12, 13, 14, 15, 17, 18, 19, 20, 22, 23, 24, 25];
    // 각 라인별 남아있는 매물
    room.remain_lineup = [
        [2, 3, 4, 5],
        [7, 8, 9, 10],
    ...
    room.readyNextLineup = function(socketId) {
        ...
        // 체크된 숫자가 6보다 적으면
        if (room.ready_num < PLAYER_COUNT) {
    ...
    room.changeMoney = function(socketId, money) {
        ...
        // 경매가 시작되지 않은 경우
        if(!room.is_auction_start) { 
            console.log("경매 시작 X");
            return; } 
        // 입력된 숫자가 숫자가 아니거나 0 이하일 경우
        else if(money <= 0 || !Number.isInteger(money)) { 
            console.log("숫자 타입");
            return; } 
        // 이미 입찰자로 등록된 경우
        else if(index_of_who == room.who) { 
            console.log("입찰자");
            return; } 
        // 이미 입찰된 가격보다 낮은 경우
        else if(money <= room.money) { 
            console.log("가격 낮음");
            return; }
        // 보유 돈보다 많은 숫자를 입력했을 경우
        else if(money > room.players[index_of_who].money) { 
            console.log("많은 숫자");
            return; }
        // 이미 보유한 포지션인 경우
        else if(room.players[index_of_who].lineup_list[room.position] != -1) { 
            console.log("보유 포지션");
            return; }
        else {
            // 새로운 입찰된 돈과 사람을 업데이트
    ...
    room.PlayerOfflined = function(socketId) {
        ...
        // n명 이상 나간 경우 방을 해체
        if(room.offline_list.length >= PLAYER_COUNT) {
